...
The top three models form a cluster whose intervals overlap substantially, and this study should not be cited as evidence of a resolved ordering among them. The more informative separation is between this leading cluster and the open-weight tail. Notably, \model{GPT-5.5} sits below the cluster here, even though the OpenAI family led a sibling \emph{text} dental benchmark by the same authors; being strongest at text did not transfer to being strongest at vision. A sensitivity analysis excluding the 10 pre-flagged weak or ambiguous items (n$=$80) shifted every model upward by 3 to 5 points and left the ranking unchanged.
...
The two judges returned the same pass/fail verdict on 82\% of descriptions, with Cohen's $\kappa=0.65$ (Table~\ref{tab:judges}). The primary judge (\model{Claude Opus 4.8}) was more lenient than the secondary judge (\model{GPT-5.5}) for every model. Critically, there was no own-family advantage in these deltas: \model{Claude Opus 4.8} had the \emph{smallest} primary-versus-secondary gap (+7 points), not the largest, which argues against a simple self-preference explanation and reinforces that absolute pass rates should be read as judge-dependent. A single-pass delta cannot rule out subtler bias.
