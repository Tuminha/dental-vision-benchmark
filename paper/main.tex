...
\begin{figure}[t]
\centering
\includegraphics[width=0.7\linewidth]{figures/pairwise.pdf}
...
A mixed-effects logistic regression with a random intercept per image, and fixed effects for model, modality, labeled status, and weak-item flag, clarifies the task split. The strongest controllable factor is text labeling: labeled images have 2.2 times the odds of a passing description (95\% credible interval [1.6, 3.1]) relative to unlabeled images. Once labeling and item difficulty are accounted for, the diagram-versus-clinical-photograph difference is not significant (odds ratio 0.84, [0.59, 1.19]), so the apparent diagram advantage in Table~\ref{tab:results} is largely explained by diagrams more often carrying readable labels. Weak or ambiguous items are much harder, as expected (odds ratio 0.12, [0.05, 0.25]). Finally, rubric burden does not explain difficulty: the pooled pass rate is flat to non-monotonic from 2 to 6 required points, and an item-clustered logistic regression finds no significant effect of the number of \emph{must identify} points (odds ratio 1.23 per point, [0.82, 1.83], $p=0.32$), so the benchmark is not simply penalizing longer rubrics.
